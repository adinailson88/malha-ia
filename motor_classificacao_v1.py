...
# =====================================================================
# MAPEAMENTO DE COLUNAS — LEITURA COM get_all_values()
# Índice Python começa em 0.
# =====================================================================
COL_TITULO                = 1   # B
...
IDX_AVALIACAO             = 27  # AB
IDX_EXECUTOR              = 28  # AC
IDX_CRITICIDADE           = 29  # AD
IDX_CONFERENCIA           = 31  # AF

# Alias mantido para compatibilidade com a classificação rápida já existente
COL_CAT_IA                = IDX_CAT_IA
COL_CONFERENCIA           = IDX_CONFERENCIA

# =====================================================================
# MAPEAMENTO DE COLUNAS — ESCRITA COM gspread.Cell()
# Número de coluna do Google Sheets começa em 1.
# =====================================================================
COL_CAT_IA_OUT            = 26  # Z
...
def _erro_transitorio(e):
    """True para erros temporarios do backend Google (nao adianta abortar o run):
    429 (rate limit) e 5xx (500/502/503/504, 'service currently unavailable')."""
...
def _conectar_planilha(tentativas=5, espera_base=15):
    """Abre a planilha com retry/backoff. O agendado (*/15) caia por APIError
    [503] transitorio do Google ja na 1a chamada (open_by_key); aqui reententamos
    com espera crescente antes de tratar como erro real."""
    ultimo = None
...
def extrair_nome_executor(origem):
    ...
    mapa = {
        ...
        "Reclass_LSTM":                   "Reclass_LSTM",
        "Reclass_RF":                     "Reclass_RF",
...
def _valor_linha(linha, idx, padrao=''):
    """
    Lê uma célula de uma linha retornada por get_all_values().
    get_all_values() usa índice Python começando em 0.
    """
    if len(linha) > idx:
...
def _normalizar_conf_planilha(valor):
    """
    Converte confiança gravada na planilha para escala decimal.
    ...
    Retorna:
    - float em escala 0 a 1
    - None quando não for possível converter
    """
    if valor is None:
        return None
...
def _conferencia_humana_marcada(valor):
    """
    Verifica se a coluna AF indica conferência humana.
    Se marcada, o motor não pode sobrescrever a linha.
    """
    marcado = str(valor or '').strip().upper()
    return marcado in ('TRUE', 'VERDADEIRO', '1', 'SIM')


def _executor_baixa_confianca(executor):
    """
    Identifica classificações antigas feitas por caminho de baixa confiança
    ou fallback que devem ser elegíveis à reclassificação.
    """
    ...
    return executor_norm in (
        ...
        'RF_FALLBACK',
        'SUPERVISIONADO_LSTM_BAIXA_CONF',
        'SUPERVISIONADO_LSTM_BAIXA_CONF.',
        'RECLASS_LSTM_BAIXA_CONF',
        'RECLASS_RF_BAIXA_CONF',
    )


def _categoria_valida(cat):
    """
    Categoria válida para substituição operacional.
    PENDENTE_REVISAO não é considerada categoria final válida.
    """
    cat = str(cat or '').strip()
    return bool(cat) and cat != 'PENDENTE_REVISAO'


def _modo_reclassificacao():
    """
    Reavalia chamados já classificados quando houver indício de baixa confiança,
    fallback ou pendência de revisão.

    Regras principais:
    - respeita AF/conferência humana;
    - usa índices separados para leitura e escrita;
    - só altera Z/AB/AC/AD quando a nova classificação atende aos critérios;
    - registra decisão clara em LOG_CLASSIFICACAO.
    """
    ...
    for i, linha in enumerate(todas_linhas):
        ...
        cat_atual = _valor_linha(linha, IDX_CAT_IA)
        if not cat_atual:
            continue

        conferencia = _valor_linha(linha, IDX_CONFERENCIA)
        if _conferencia_humana_marcada(conferencia):
            continue

        conf_antiga_raw = _valor_linha(linha, IDX_AVALIACAO)
        conf_antiga = _normalizar_conf_planilha(conf_antiga_raw)

        executor_antigo = _valor_linha(linha, IDX_EXECUTOR)

        motivos_elegibilidade = []

        if conf_antiga is not None and conf_antiga < LIMIAR_RECLASSIFICACAO:
            motivos_elegibilidade.append(f"AB<{LIMIAR_RECLASSIFICACAO:.2f}")

        if _executor_baixa_confianca(executor_antigo):
            motivos_elegibilidade.append(f"AC={executor_antigo}")

        if cat_atual == "PENDENTE_REVISAO":
            motivos_elegibilidade.append("Z=PENDENTE_REVISAO")

        if not motivos_elegibilidade:
            continue
        ...
        cat_orig = _valor_linha(linha, COL_CATEGORIA_HIERARQUICA)

        candidatos.append({
            'num_linha': num_linha,
            'texto': texto,
            'cat_original': cat_orig,
            'cat_atual': cat_atual,
            'conf_antiga': conf_antiga,
            'conf_antiga_raw': conf_antiga_raw,
            'executor_antigo': executor_antigo,
            'motivo_elegibilidade': '; '.join(motivos_elegibilidade),
        })
    ...
    print(f"[Modo reclassificacao] {len(candidatos)} candidato(s) elegível(is).")
    ...
    for c in candidatos:
        ...
        conf_antiga = c['conf_antiga'] if c['conf_antiga'] is not None else 0.0
        executor_antigo = c['executor_antigo']
        cat_atual = c['cat_atual']

        sobrescrever = False
        motivo = ""

        if nova_cat == "PENDENTE_REVISAO":
            motivo = "nova_cat=PENDENTE_REVISAO; mantido"
            sobrescrever = False

        elif cat_atual == "PENDENTE_REVISAO" and _categoria_valida(nova_cat) and nova_conf >= 0.70:
            motivo = "atual=PENDENTE_REVISAO; nova válida com conf>=0,70"
            sobrescrever = True

        elif nova_cat != cat_atual and nova_conf >= 0.80:
            motivo = "nova categoria diferente com conf>=0,80"
            sobrescrever = True

        elif nova_cat == cat_atual and nova_conf >= conf_antiga + DELTA_MELHORIA_MINIMA:
            motivo = f"mesma categoria com melhoria>= {DELTA_MELHORIA_MINIMA:.2f}"
            sobrescrever = True

        elif _executor_baixa_confianca(executor_antigo) and nova_conf >= 0.75:
            motivo = "executor antigo baixa confiança; nova conf>=0,75"
            sobrescrever = True

        else:
            motivo = (
                "não atende critério de sobrescrita; "
                f"elegibilidade={c['motivo_elegibilidade']}"
            )
        ...
        decisao = (
            f"Reclass: {cat_atual}→{nova_cat} "
            f"({conf_antiga:.2f}→{nova_conf:.2f}) [{motivo}]"
        )
        ...
        celulas_update += [
            ...
            gspread.Cell(num, COL_EXECUTOR_OUT,     nome_origem),
            gspread.Cell(num, COL_CRITICIDADE_OUT,  crit),
        ]
    ...
    if celulas_update:
        try:
            ...
            print(
                f"[Modo reclassificacao] {n_alterados} reclassificado(s), "
        ...
        print(
            f"[Modo reclassificacao] Nenhuma alteração aplicável. "
            f"{n_inalterados} candidato(s) mantido(s)."
        )
